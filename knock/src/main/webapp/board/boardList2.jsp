...
p.setTotalRecord(BoardDAO.getTotalCount2());
p.setTotalPage();
...
List<SubtitleVO> list = BoardDAO.getList2(p.getBegin(), p.getEnd());
					...
					<tr>
						<td colspan="4">
							<ol class="paging">
								...
								<c:if test="${pvo.beginPage != 1 }">
									<li><a
										href="${contextPath }/boardList2.jsp?cPage=${pvo.beginPage - 1 }">이전</a>
									</li>
								</c:if>
								<c:forEach var="pageNo" begin="${pvo.beginPage}"
									...
									<c:choose>
										...
										<c:otherwise>
											<li><a
												href="${contextPath }/board?type=boardList2&cPage=${pageNo }">${pageNo }</a>
											</li>
										</c:otherwise>
									</c:choose>
								</c:forEach>
								...
								<c:if test="${pvo.endPage < pvo.totalPage }">
									<li><a
										href="${contextPath }/board?type=boardList2&cPage=${pvo.endPage + 1 }">다음</a></li>
								</c:if>
							</ol>
						</td>
					</tr>
